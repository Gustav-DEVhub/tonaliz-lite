...
export function useTrackShelves(configs: TrackShelfConfig[], enabled = true, revision = 0) {
  const cacheKey = `${getShelfCacheKey(configs)}::${revision}`
  ...
  useEffect(() => {
    ...
    void loadTrackShelves(configs, revision)
      .then((shelves) => {
        if (isCancelled) {
          return
        }
  ...
  }, [cacheKey, configs, enabled, revision])
